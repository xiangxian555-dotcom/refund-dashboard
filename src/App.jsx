...
const MAIN_TABS = ["환불 현황","처리 유형","날짜별 대응현황","계정별 대응현황","퍼널 통계","분석 리포트","프로세스 안내","유저 조회"];
  ...
  if (ci.date < 0) {
    ...
    for (let i = 1; i < Math.min(11, allRows.length); i++) {
      allRows[i].forEach((cell, idx) => {
        if (dateLikePattern.test(String(cell || ""))) colScores[idx] = (colScores[idx] || 0) + 1;
      });
    }
...
export default function App() {
  ...
  const parseFile = useCallback(async (file) => {
    ...
    try {
        ...
        if(isRefund) {
          ...
          rows.forEach(r => {
            ...
            refRows.push({
              ...
              uc:parseUC(product), financialStatus:String(r["Financial Status"]||"").toLowerCase(),
      ...
      setSheetLog(log); setRefundData(refRows); setProcessData(procRows);
      if(!refRows.length && !procRows.length) setUploadErr("데이터를 인식하지 못했어요.");
  ...
  const fetchSheet = useCallback(async () => {
    ...
    for (const cfg of SHEET_TARGETS) {
      ...
      try {
        ...
        if (res.ok) { const text = await res.text(); if (text && text.length > 10) csvText = { text, method:"Viz API" }; }
      } catch(e) {}
      if (!csvText) {
        try {
          ...
          if (res.ok) { const text = await res.text(); if (text && text.length > 10) csvText = { text, method:"직접 연결" }; }
        } catch(e) {}
      }
      if (!csvText) {
        try {
          ...
          if (res.ok) { const text = await res.text(); if (text && text.length > 10) csvText = { text, method:"CORS 프록시" }; }
        } catch(e) {}
      }
        ...
        if (!currentColMap) {
          ...
          try {
            ...
            if (aiRes.ok) {
              ...
              if (aiData.columns) { currentColMap = aiData.columns; setColMap(currentColMap); setAiStatus("✅ AI 분석 완료"); }
            }
          } catch(e) { setAiStatus("⚠️ AI 분석 실패 — 기본 매핑 사용"); }
        }
  ...
  const merged = useMemo(() => {
    ...
    return refundData.map(r => {
      ...
      return { ...r, type:p.type||"회수처리", abuseCount:p.abuseCount||0, totalUC:p.totalUC||r.uc, currentUC:p.currentUC||0, pValue:p.pValue??0, result:p.result||"" };
    });
  ...
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  // 날짜별 집계 (행 기준 — 중복 openid 포함)
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  const dateSummary = useMemo(() => ({
    totalRows: respFiltered.length,
    recovered: respFiltered.filter(d => d.status === "복구완료").length,
    processing: respFiltered.filter(d => d.status === "처리중").length,
    resanctioned: respFiltered.filter(d => d.status === "재제재").length,
  }), [respFiltered]);

  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  // 계정별 집계 (openid 유니크 기준 — 최신 날짜 상태)
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  const uniqueOpenids = useMemo(() => {
    const map = {};
    const sorted = [...respFiltered].sort((a, b) => a.date.localeCompare(b.date));
    sorted.forEach(d => {
      if (!map[d.openid]) {
        map[d.openid] = { status: d.status, resanctioned: d.resanctioned, amount: 0, lastDate: d.date, platform: d.platform, country: d.country };
      }
      if (d.date >= map[d.openid].lastDate) {
        map[d.openid].status = d.status;
        map[d.openid].resanctioned = d.resanctioned;
  ...
  const piePlatform = useMemo(()=>{ const c={}; filtered.forEach(d=>{c[d.platform]=(c[d.platform]||0)+1;}); return Object.entries(c).map(([k,v])=>({name:k,value:v,color:PC[k]})); },[filtered]);
  const pieType = useMemo(()=>{ const c={}; filtered.forEach(d=>{c[d.type]=(c[d.type]||0)+1;}); return Object.entries(c).map(([k,v])=>({name:k,value:v,color:TC[k]})); },[filtered]);

  const pieDate = useMemo(()=>[
    {name:"복구완료", value:dateSummary.recovered, color:"#22c55e"},
    {name:"처리중", value:dateSummary.processing, color:"#22d3ee"},
    {name:"재제재", value:dateSummary.resanctioned, color:"#ef4444"},
  ].filter(d=>d.value>0),[dateSummary]);

  const pieAccount = useMemo(()=>[
    {name:"복구완료", value:accountSummary.recovered, color:"#22c55e"},
    {name:"처리중", value:accountSummary.processing, color:"#22d3ee"},
    {name:"재제재", value:accountSummary.resanctioned, color:"#ef4444"},
  ].filter(d=>d.value>0),[accountSummary]);
  ...
  const funnelStats = useMemo(()=>[
    ...
    {name:"복구 문의", value:accountSummary.uniqueUsers, fill:"#f59e0b"},
    {name:"복구 완료", value:accountSummary.recovered, fill:"#22c55e"},
  ...
  const UploadBox = () => (
    <div style={{background:"#1e293b",borderRadius:12,padding:16,marginBottom:12}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
        <span style={{fontSize:13,fontWeight:600,color:"#e2e8f0"}}>📁 엑셀 업로드</span>
      ...
      {hasData ? (
        ...
        <div onDrop={onDrop} onDragOver={e=>e.preventDefault()}
          ...
          {uploading ? <div style={{color:"#6366f1",fontSize:13}}>⏳ 분석 중...</div> : (<>
            <div style={{fontSize:26,marginBottom:6}}>📂</div>
            <div style={{color:"#e2e8f0",fontSize:13,fontWeight:600,marginBottom:3}}>드래그 또는 클릭하여 업로드</div>
            <div style={{color:"#64748b",fontSize:12}}>시트1: 환불 원본 | 시트2: 처리결과</div>
          </>)}
  ...
  // ── Google Sheets 연결 버튼 박스 ──
  const SheetConnectBox = () => (
    <div style={{background:"#1e293b",borderRadius:12,padding:16,marginBottom:14}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
        <div>
          <div style={{fontSize:13,fontWeight:600,color:"#e2e8f0",marginBottom:4}}>🔗 Google Sheets 실시간 연동</div>
  ...
  // ── 플랫폼별 카드 (공통) ──
  const PlatformCards = ({data, byAccount=false}) => (
    <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:12,marginBottom:14}}>
      {["한국_Google","한국_iOS","일본_Google","일본_iOS"].map(key=>{
        const [c,p]=key.split("_");
        const rows = data.filter(d=>d.country===c&&d.platform===p);
        let done, ing, resan, total;
        if (byAccount) {
          const uids = {};
          const sorted = [...rows].sort((a,b)=>a.date.localeCompare(b.date));
          sorted.forEach(d=>{ if(!uids[d.openid]) uids[d.openid]={status:d.status,resanctioned:d.resanctioned,lastDate:d.date}; if(d.date>=uids[d.openid].lastDate){uids[d.openid].status=d.status;uids[d.openid].resanctioned=d.resanctioned;uids[d.openid].lastDate=d.date;} });
          const vals = Object.values(uids);
          total = vals.length;
          done = vals.filter(d=>d.status==="복구완료").length;
          ing = vals.filter(d=>d.status==="처리중").length;
          resan = vals.filter(d=>d.status==="재제재").length;
        } else {
          total = rows.length;
          done = rows.filter(d=>d.status==="복구완료").length;
          ing = rows.filter(d=>d.status==="처리중").length;
          resan = rows.filter(d=>d.status==="재제재").length;
        }
        const unit = byAccount ? "명" : "건";
        return (
          <div key={key} style={{background:"#1e293b",borderRadius:12,padding:16,borderLeft:`4px solid ${p==="Google"?"#4285F4":"#A855F7"}`}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:12}}>
              ...
              <span style={{fontSize:11,color:"#64748b"}}>{byAccount ? `${fmt(rows.length)}행 · ${fmt(total)}명` : `${fmt(total)}건`}</span>
            </div>
            {[["복구완료",done,"#22c55e"],["처리중",ing,"#22d3ee"],["재제재",resan,"#ef4444"]].map(([l,v,col])=>(
              <div key={l} style={{marginBottom:8}}>
                <div style={{display:"flex",justifyContent:"space-between",fontSize:11,marginBottom:3}}>
                  <span style={{color:"#64748b"}}>{l}</span>
                  <span style={{color:col,fontWeight:600}}>{fmt(v)}{unit} ({total?Math.round(v/total*100):0}%)</span>
                </div>
  ...
  return (
    <div style={{fontFamily:"sans-serif",background:"#0f172a",minHeight:"100vh",color:"#e2e8f0",padding:"16px"}}>
      ...
      {/* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
          날짜별 대응현황 — 행 기준, 중복 openid 포함
          월별/주별 처리 건수 파악용
      ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */}
      {tab==="날짜별 대응현황" && (<>
        <div style={{background:"#1e293b",borderRadius:12,padding:12,marginBottom:14,borderLeft:"4px solid #22d3ee"}}>
          <div style={{fontSize:12,color:"#22d3ee",fontWeight:600,marginBottom:2}}>📅 날짜별 대응현황</div>
          <div style={{fontSize:11,color:"#64748b"}}>Google Sheets 행 기준 집계 — 같은 유저가 여러 번 문의한 경우 각각 카운트 · 월별/주별 처리 건수 파악에 적합</div>
        </div>
        <SheetConnectBox/>
        {responseData.length === 0 ? (
          <div style={{background:"#1e293b",borderRadius:12,padding:32,textAlign:"center",color:"#64748b",fontSize:13}}>위 버튼을 클릭하면 Google Sheets에서 실시간 데이터를 불러옵니다.</div>
        ) : (<>
          {filterBar(true, respPeriod, setRespPeriod)}
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(110px,1fr))",gap:10,marginBottom:14}}>
            <Card icon="📋" label="총 처리건수" value={fmt(dateSummary.totalRows)} sub="행 기준 (중복 포함)" color="#6366f1"/>
            <Card icon="✅" label="복구완료" value={fmt(dateSummary.recovered)} sub={`${dateSummary.totalRows?Math.round(dateSummary.recovered/dateSummary.totalRows*100):0}%`} color="#22c55e"/>
            <Card icon="⏳" label="처리중" value={fmt(dateSummary.processing)} sub={`${dateSummary.totalRows?Math.round(dateSummary.processing/dateSummary.totalRows*100):0}%`} color="#22d3ee"/>
            <Card icon="🚫" label="재제재" value={fmt(dateSummary.resanctioned)} sub={`${dateSummary.totalRows?Math.round(dateSummary.resanctioned/dateSummary.totalRows*100):0}%`} color="#ef4444"/>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"2fr 1fr",gap:12,marginBottom:14}}>
            <div style={{background:"#1e293b",borderRadius:12,padding:16}}>
              <div style={{fontSize:13,color:"#94a3b8",marginBottom:12}}>날짜별 처리 추이 (행 기준)</div>
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={respTrendData}><CartesianGrid strokeDasharray="3 3" stroke="#334155"/>
                  ...
                  <Bar dataKey="처리중" fill="#22d3ee" stackId="a"/>
                  <Bar dataKey="재제재" fill="#ef4444" radius={[3,3,0,0]} stackId="a"/>
                </BarChart>
              </ResponsiveContainer>
            </div>
            <div style={{background:"#1e293b",borderRadius:12,padding:16}}>
              <div style={{fontSize:13,color:"#94a3b8",marginBottom:10}}>처리 비율 (건수 기준)</div>
              <ResponsiveContainer width="100%" height={140}><PieChart>
                <Pie data={pieDate} cx="50%" cy="50%" innerRadius={35} outerRadius={58} dataKey="value" paddingAngle={3}>
                  {pieDate.map((e,i)=><Cell key={i} fill={e.color}/>)}</Pie><Tooltip {...TT}/>
              </PieChart></ResponsiveContainer>
              {pieDate.map(d=>(<div key={d.name} style={{display:"flex",alignItems:"center",gap:6,fontSize:12,marginTop:6}}>
                <div style={{width:8,height:8,borderRadius:"50%",background:d.color}}/><span style={{color:"#94a3b8"}}>{d.name}</span>
                <span style={{marginLeft:"auto",fontWeight:600}}>{fmt(d.value)}건</span></div>))}
            </div>
          </div>
          <PlatformCards data={respFiltered} byAccount={false}/>
        </>)}
      </>)}

      {/* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
          계정별 대응현황 — openid 유니크 기준, 최신 상태
          실제 몇 명인지 파악용
      ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */}
      {tab==="계정별 대응현황" && (<>
        <div style={{background:"#1e293b",borderRadius:12,padding:12,marginBottom:14,borderLeft:"4px solid #a855f7"}}>
          <div style={{fontSize:12,color:"#a855f7",fontWeight:600,marginBottom:2}}>👤 계정별 대응현황</div>
          <div style={{fontSize:11,color:"#64748b"}}>OpenID 유니크 기준 집계 — 동일 유저의 최신 처리 상태로 카운트 · 실제 계정 수 파악에 적합</div>
        </div>
        <SheetConnectBox/>
        {responseData.length === 0 ? (
          <div style={{background:"#1e293b",borderRadius:12,padding:32,textAlign:"center",color:"#64748b",fontSize:13}}>위 버튼을 클릭하면 Google Sheets에서 실시간 데이터를 불러옵니다.</div>
          ...
          {filterBar(false)}
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(110px,1fr))",gap:10,marginBottom:14}}>
            <Card icon="👥" label="총 유저수" value={fmt(accountSummary.uniqueUsers)} sub="계정 기준 (중복 제거)" color="#a855f7"/>
            <Card icon="✅" label="복구완료" value={fmt(accountSummary.recovered)} sub={`${accountSummary.uniqueUsers?Math.round(accountSummary.recovered/accountSummary.uniqueUsers*100):0}%`} color="#22c55e"/>
            <Card icon="⏳" label="처리중" value={fmt(accountSummary.processing)} sub={`${accountSummary.uniqueUsers?Math.round(accountSummary.processing/accountSummary.uniqueUsers*100):0}%`} color="#22d3ee"/>
            <Card icon="🚫" label="재제재" value={fmt(accountSummary.resanctioned)} sub={`${accountSummary.uniqueUsers?Math.round(accountSummary.resanctioned/accountSummary.uniqueUsers*100):0}%`} color="#ef4444"/>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"2fr 1fr",gap:12,marginBottom:14}}>
            <div style={{background:"#1e293b",borderRadius:12,padding:16}}>
              <div style={{fontSize:13,color:"#94a3b8",marginBottom:4}}>계정 상태 분포</div>
              <div style={{fontSize:11,color:"#64748b",marginBottom:12}}>각 유저의 최신 처리 상태 기준</div>
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                {[["복구완료",accountSummary.recovered,"#22c55e","✅"],
                  ["처리중",accountSummary.processing,"#22d3ee","⏳"],
                  ["재제재",accountSummary.resanctioned,"#ef4444","🚫"]].map(([l,v,col,icon])=>(
                  <div key={l}>
                    <div style={{display:"flex",justifyContent:"space-between",fontSize:12,marginBottom:4}}>
                      <span style={{color:"#94a3b8"}}>{icon} {l}</span>
                      <span style={{color:col,fontWeight:700}}>{fmt(v)}명 ({accountSummary.uniqueUsers?Math.round(v/accountSummary.uniqueUsers*100):0}%)</span>
                    </div>
                    <div style={{background:"#0f172a",borderRadius:6,height:10}}>
                      <div style={{width:`${accountSummary.uniqueUsers?Math.round(v/accountSummary.uniqueUsers*100):0}%`,height:"100%",background:col,borderRadius:6}}/>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div style={{background:"#1e293b",borderRadius:12,padding:16}}>
              <div style={{fontSize:13,color:"#94a3b8",marginBottom:10}}>계정 비율</div>
              <ResponsiveContainer width="100%" height={140}><PieChart>
                <Pie data={pieAccount} cx="50%" cy="50%" innerRadius={35} outerRadius={58} dataKey="value" paddingAngle={3}>
                  {pieAccount.map((e,i)=><Cell key={i} fill={e.color}/>)}</Pie><Tooltip {...TT}/>
              </PieChart></ResponsiveContainer>
              {pieAccount.map(d=>(<div key={d.name} style={{display:"flex",alignItems:"center",gap:6,fontSize:12,marginTop:6}}>
                <div style={{width:8,height:8,borderRadius:"50%",background:d.color}}/><span style={{color:"#94a3b8"}}>{d.name}</span>
                <span style={{marginLeft:"auto",fontWeight:600}}>{fmt(d.value)}명</span></div>))}
            </div>
          </div>
          <PlatformCards data={respFiltered} byAccount={true}/>
        </>)}
      </>)}
      ...
      {tab==="분석 리포트" && (<>
        ...
        <div style={{background:"linear-gradient(135deg, #1e293b 0%, #0f172a 100%)",borderRadius:16,padding:24,marginBottom:14,border:"1px solid #334155"}}>
          ...
          <div style={{display:"flex",gap:0,alignItems:"center",justifyContent:"center",flexWrap:"wrap"}}>
            {[
              ...
              {label:"복구 문의", value:fmt(accountSummary.uniqueUsers), sub:`명 (정지자의 ${summary.sanction?Math.round(accountSummary.uniqueUsers/summary.sanction*100):0}%)`, color:"#f59e0b", icon:"📞"},
              {label:"복구 완료", value:fmt(accountSummary.recovered), sub:`명 (문의자의 ${accountSummary.uniqueUsers?Math.round(accountSummary.recovered/accountSummary.uniqueUsers*100):0}%)`, color:"#22c55e", icon:"✅"},
              {label:"재제재", value:fmt(accountSummary.resanctioned), sub:`명 (문의자의 ${accountSummary.uniqueUsers?Math.round(accountSummary.resanctioned/accountSummary.uniqueUsers*100):0}%)`, color:"#ef4444", icon:"🚫"},
            ].map((item,idx)=>(
        ...
        <div style={{background:"#1e293b",borderRadius:12,padding:20,marginBottom:14}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
            ...
            <button onClick={()=>{
              const text = `[환불 대응 분석 리포트]\n기간: ${from} ~ ${to}\n\n` +
                ...
                `3. 복구 문의: ${fmt(accountSummary.uniqueUsers)}명 (정지자의 ${summary.sanction?Math.round(accountSummary.uniqueUsers/summary.sanction*100):0}%)\n` +
                `4. 복구 완료: ${fmt(accountSummary.recovered)}명 (문의자의 ${accountSummary.uniqueUsers?Math.round(accountSummary.recovered/accountSummary.uniqueUsers*100):0}%)\n` +
                `5. 재제재 (미재결제): ${fmt(accountSummary.resanctioned)}명 (문의자의 ${accountSummary.uniqueUsers?Math.round(accountSummary.resanctioned/accountSummary.uniqueUsers*100):0}%)\n\n` +
                `[요약]\n${from}~${to} 기간 동안 총 ${fmt(summary.total)}명이 앱 마켓 환불을 진행하였으며, ` +
                `그 중 ${fmt(summary.sanction)}명의 계정이 정지되었습니다. ` +
                `정지된 계정 중 ${fmt(accountSummary.uniqueUsers)}명이 복구를 문의하였고, ` +
                `${fmt(accountSummary.recovered)}명이 재결제 후 복구 완료되었습니다. ` +
                `${fmt(accountSummary.resanctioned)}명은 재결제를 하지 않아 재제재되었습니다.`;
          ...
          <div style={{background:"#0f172a",borderRadius:8,padding:16,fontSize:13,lineHeight:1.8,color:"#e2e8f0"}}>
            ...
            <div style={{marginBottom:6}}>3. <span style={{color:"#f59e0b"}}>복구 문의</span>: <strong>{fmt(accountSummary.uniqueUsers)}</strong>명 <span style={{color:"#64748b"}}>(정지자의 {summary.sanction?Math.round(accountSummary.uniqueUsers/summary.sanction*100):0}%)</span></div>
            <div style={{marginBottom:6}}>4. <span style={{color:"#22c55e"}}>복구 완료</span>: <strong>{fmt(accountSummary.recovered)}</strong>명 <span style={{color:"#64748b"}}>(문의자의 {accountSummary.uniqueUsers?Math.round(accountSummary.recovered/accountSummary.uniqueUsers*100):0}%)</span></div>
            <div style={{marginBottom:12}}>5. <span style={{color:"#ef4444"}}>재제재</span>: <strong>{fmt(accountSummary.resanctioned)}</strong>명 <span style={{color:"#64748b"}}>(문의자의 {accountSummary.uniqueUsers?Math.round(accountSummary.resanctioned/accountSummary.uniqueUsers*100):0}%)</span></div>
            <div style={{borderTop:"1px solid #334155",paddingTop:12,color:"#94a3b8",fontSize:12}}>
              ...
              정지된 계정 중 <strong style={{color:"#f59e0b"}}>{fmt(accountSummary.uniqueUsers)}명</strong>이 복구를 문의하였고,
              <strong style={{color:"#22c55e"}}>{fmt(accountSummary.recovered)}명</strong>이 재결제 후 복구 완료되었습니다.
              <strong style={{color:"#ef4444"}}>{fmt(accountSummary.resanctioned)}명</strong>은 재결제를 하지 않아 재제재되었습니다.
            </div>
          </div>
        </div>
      </>)}
      ...
      {tab==="프로세스 안내" && (<>
        ...
        <div style={{background:"#1e293b",borderRadius:12,padding:20,marginBottom:14,borderLeft:"4px solid #ef4444"}}>
          ...
          <div style={{fontSize:13,color:"#e2e8f0",lineHeight:1.8,marginBottom:12}}>
            고객센터를 통하지 않고 <strong style={{color:"#f59e0b"}}>앱 마켓을 통해 직접 환불</strong>을 진행한 경우,
            <strong style={{color:"#22d3ee"}}>재화(UC)의 사용 여부를 검토</strong> 후 조치합니다.
          </div>
          <div style={{background:"#0f172a",borderRadius:8,padding:14,display:"flex",gap:12,alignItems:"center"}}>
            ...
            <div style={{fontSize:12,color:"#f59e0b",lineHeight:1.6}}>
              <strong>핵심:</strong> 환불은 받았으나 재화는 이미 사용 → <strong>무료로 유료 콘텐츠를 이용</strong>한 것이므로 계정 정지
            </div>
          </div>
        </div>
        <div style={{background:"#1e293b",borderRadius:12,padding:20,marginBottom:14,borderLeft:"4px solid #f59e0b"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:0}}>
            {[
              {step:"1",title:"환불 데이터 추출",desc:"앱 마켓에서 환불 데이터를 추출합니다.",icon:"📱"},
              {step:"2",title:"재화(UC) 사용 여부 확인",desc:"해당 결제로 지급된 재화의 사용 여부를 확인합니다.",icon:"🔍"},
              {step:"3-1",title:"재화 보유 시 → 재화 회수",desc:"재화를 아직 보유 중이면 회수하고 계정 정지 없음.",icon:"✅",color:"#22c55e"},
              {step:"3-2",title:"재화 사용 시 → 계정 정지",desc:"재화를 이미 사용했으면 계정을 정지합니다.",icon:"🔒",color:"#ef4444"},
            ].map((item,i)=>(
              <div key={i} style={{display:"flex",gap:14,alignItems:"flex-start"}}>
                <div style={{display:"flex",flexDirection:"column",alignItems:"center"}}>
                  <div style={{width:36,height:36,borderRadius:"50%",background:item.color||"#334155",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,fontWeight:700,color:"#fff",flexShrink:0}}>{item.step}</div>
        ...
        <div style={{background:"#1e293b",borderRadius:12,padding:20,marginBottom:14,borderLeft:"4px solid #22c55e"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:0}}>
            {[
              ...
              {step:"3",title:"재결제 필요 안내",desc:"환불받은 금액만큼 재결제가 필요함을 안내합니다.",icon:"💳"},
              {step:"4",title:"이용자 재결제 완료",desc:"유저가 안내받은 상품을 재결제합니다.",icon:"✅"},
              {step:"5",title:"재화 회수 및 계정 해제",desc:"재결제로 지급된 재화를 회수하고 계정 정지를 해제합니다.",icon:"🎉"},
            ].map((item,i)=>(
              <div key={i} style={{display:"flex",gap:14,alignItems:"flex-start"}}>
                <div style={{display:"flex",flexDirection:"column",alignItems:"center"}}>
                  <div style={{width:36,height:36,borderRadius:"50%",background:"#22c55e",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,fontWeight:700,color:"#fff",flexShrink:0}}>{item.step}</div>
        ...
        <div style={{background:"#1e293b",borderRadius:12,padding:20,borderLeft:"4px solid #6366f1"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12}}>
            {[
              {status:"복구완료",desc:"재화 회수 완료 / 계정 해제 / 처리가 완료된 건",color:"#22c55e",icon:"✅",keywords:"회수, 완료, 해제"},
      ...
      <div style={{marginTop:14,padding:"10px 14px",background:"#1e293b",borderRadius:8,fontSize:11,color:"#475569",borderLeft:"3px solid #6366f1"}}>
        💡 날짜별: 행 기준 처리 건수 · 계정별: OpenID 유니크 기준 · 유저 조회: 개별 히스토리 전체 확인
      </div>
    </div>
  );
}
...
function UserSearch({refundData, responseData}) {
  ...
  const search = () => {
    ...
    const responses = responseData
      .filter(d => String(d.openid||"").includes(s))
      .sort((a,b) => a.date.localeCompare(b.date)); // 날짜 오름차순 (히스토리 순)
    setRes({
      q: s,
  ...
  // 최종 상태 계산 (최신 날짜 기준)
  const finalStatus = res?.responses?.length
    ? res.responses[res.responses.length-1].status
    : null;
  const finalColor = finalStatus==="복구완료"?"#22c55e":finalStatus==="재제재"?"#ef4444":"#22d3ee";

  return (
    <div>
      <div style={{background:"#1e293b",borderRadius:12,padding:12,marginBottom:14,borderLeft:"4px solid #6366f1"}}>
        <div style={{fontSize:12,color:"#6366f1",fontWeight:600,marginBottom:2}}>🔍 유저 조회</div>
        <div style={{fontSize:11,color:"#64748b"}}>OpenID로 검색하면 환불 내역 + Google Sheets의 모든 대응 히스토리를 날짜순으로 확인할 수 있습니다.</div>
      </div>
          ...
          <div style={{background:"#1e293b",borderRadius:12,padding:16,borderLeft:"4px solid #6366f1"}}>
            ...
            <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
              {[
                ...
                ["대응이력", res.responses.length+"건", "#f59e0b"],
                ["현재 상태", finalStatus||"-", finalColor],
              ].map(([l,v,c])=>(
                <div key={l} style={{background:"#0f172a",borderRadius:8,padding:"8px 14px"}}>
                      ...
                      <tr key={i} style={{borderBottom:"1px solid #0f172a",background:i%2===0?"#0f172a":"transparent"}}>
                        ...
                        <td style={{padding:"7px 10px"}}>
                          <span style={{padding:"2px 7px",borderRadius:4,fontSize:10,
                            ...
                            color:r.platform==="Google"?"#4285F4":"#A855F7"}}>{r.platform}</span>
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          {res.responses.length>0 && (
            <div style={{background:"#1e293b",borderRadius:12,padding:16}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
                <div style={{fontSize:13,color:"#94a3b8"}}>📋 대응 히스토리 전체</div>
                <span style={{fontSize:11,color:"#64748b"}}>총 {res.responses.length}건 · 날짜순</span>
              </div>
              <div style={{display:"flex",flexDirection:"column",gap:0}}>
                {res.responses.map((r,i)=>{
                  const col = r.resanctioned?"#ef4444":r.status==="복구완료"?"#22c55e":"#22d3ee";
                  const icon = r.resanctioned?"🚫":r.status==="복구완료"?"✅":"⏳";
                  const isLast = i === res.responses.length-1;
                  return (
                    <div key={i} style={{display:"flex",gap:14,alignItems:"flex-start"}}>
                      <div style={{display:"flex",flexDirection:"column",alignItems:"center"}}>
                        <div style={{width:32,height:32,borderRadius:"50%",
                          background: isLast ? col : col+"33",
                          border:`2px solid ${col}`,
                          display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,flexShrink:0}}>
                          {icon}
                        </div>
                        {!isLast && <div style={{width:2,height:40,background:"#334155"}}/>}
                      </div>
                      <div style={{paddingBottom:!isLast?12:0,flex:1,
                        background: isLast ? col+"11" : "transparent",
                        borderRadius: isLast ? 8 : 0,
                        padding: isLast ? "8px 10px" : "0 0 12px 0",
                        ...
                        <div style={{display:"flex",gap:8,alignItems:"center",marginBottom:4}}>
                          ...
                          <span style={{fontSize:10,color:"#475569"}}>{r.platform}</span>
                          {isLast && <span style={{fontSize:10,color:col,fontWeight:600}}>← 최신</span>}
                        </div>
